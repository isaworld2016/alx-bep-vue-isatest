...
export default {
  ...
  computed: {
    ...
    drawerContainerClasses() {
      ...
      classes['vue-ads-z-' + this.zIndex] = true;
      classes[
        'vue-ads-pt-' + this.$parent.$data.toolbar.height
      ] = this.paddingTop;
      classes[
        'vue-ads-pb-' + this.$parent.$data.footer.height
      ] = this.paddingBottom;
      return classes;
    },
  ...
  methods: {
    ...
    updateWindow(window, oldWindow) {
      if (oldWindow === null) {
        // initial update
        if ((this.hidden === null && window === 'hidden') || this.hidden) {
          this.$emit('hide', true);
          return;
        }
        if (
